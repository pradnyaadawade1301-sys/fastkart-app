...
class _SeatMapState extends State<_SeatMap> {
  ...
  final Set<String> _booked = {'A3', 'A4', 'B6', 'B7', 'C2', 'C8', 'D1', 'D5', 'E3', 'E9', 'F4', 'F7'};

  final rows = ['A', 'B', 'C', 'D', 'E', 'F', 'G'];
  final seatsPerRow = 10;

  Color _seatColor(String seat) {
    if (_booked.contains(seat)) return Colors.grey.shade400;
    if (_selected.contains(seat)) return AppColors.primary;
    return Colors.white;
  }
    ...
    return Column(children: [
      ...
      Padding(
        ...
        child: Row(
          ...
          children: [
            const SizedBox(width: 20), // row label space
            const SizedBox(width: 4),
            ...List.generate(seatsPerRow, (i) => SizedBox(
              width: 28, // seat (24) + margin (2+2)
              child: Text(
                '${i + 1}',
            ...
            const SizedBox(width: 4),
            const SizedBox(width: 20), // right label space
          ],
        ),
      ),
      ...
      ...rows.map((row) => Padding(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            SizedBox(width: 20, child: Text(row, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.textSecondary))),
            const SizedBox(width: 4),
            ...List.generate(seatsPerRow, (i) {
              ...
              return GestureDetector(
                onTap: isBooked ? null : () {
                  ...
                  widget.onSeatsChanged(_selected.toList());
                },
                child: Container(
                  width: 24, height: 24,
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: isBooked ? Colors.grey.shade300
...
class _MovieCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => _showBookingFlow(context, movie),
      child: Container(
        decoration: BoxDecoration(
          color: Colors.white,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(movie.genre, style: const TextStyle(fontSize: 10, color: AppColors.textSecondary),
              ...
              Row(children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(colors: [AppColors.primary, Color(0xFFFFB300)]),
  ...
  void _showBookingFlow(BuildContext context, MovieModel movie) {
...
class _BookingFlowState extends State<_BookingFlow> {
      ...
      child: Column(children: [
            ...
            ClipRRect(borderRadius: BorderRadius.circular(8),
              child: Image.network(widget.movie.imageUrl, width: 45, height: 60, fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => Container(width: 45, height: 60,
                  color: _posterColor(widget.movie.title),
        ...
        Padding(
          ...
          child: Row(children: List.generate(4, (i) {
            final labels = ['Theater', 'Showtime', 'Seats', 'Confirm'];
            final done = i < _step;
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text(_step == 2 ? 'Proceed to Pay' : 'Continue',
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: ['2D', '3D', 'IMAX'].map((f) =>
        Container(
          ...
          decoration: BoxDecoration(
            color: f == '2D' ? AppColors.primary.withValues(alpha: 0.1) : Colors.white,
            borderRadius: BorderRadius.circular(8),
            border: Border.all(color: f == '2D' ? AppColors.primary : AppColors.border)),
          child: Text(f, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700,
              color: f == '2D' ? AppColors.primary : AppColors.textSecondary)),
        )
      ).toList()),
    ]);
  }
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        if (_selectedSeats.isNotEmpty)
          Container(
            ...
            child: Text('${_selectedSeats.length} selected',
      ...
      Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          _categoryChip('PREMIUM', '₹${widget.movie.price + 50}', const Color(0xFF9C27B0)),
          _categoryChip('GOLD', '₹${widget.movie.price + 20}', const Color(0xFFF57F17)),
          _categoryChip('SILVER', '₹${widget.movie.price}', const Color(0xFF455A64)),
        ]),
      ),
      const SizedBox(height: 8),
      _SeatMap(onSeatsChanged: (seats) => setState(() => _selectedSeats = seats)),
      if (_selectedSeats.isNotEmpty) ...[
        const SizedBox(height: 12),
        Container(
          padding: const EdgeInsets.all(12),
          ...
          child: Row(children: [
            ...
            Text('₹${widget.movie.price * _selectedSeats.length}',
  ...
  Widget _categoryChip(String label, String price, Color color) {
    return Column(children: [
      Container(width: 16, height: 16,
        decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(3))),
      const SizedBox(height: 3),
      Text(label, style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w700)),
      Text(price, style: const TextStyle(fontSize: 8, color: AppColors.textSecondary)),
    ]);
  }

  Widget _confirmStep() {
    final total = widget.movie.price * _selectedSeats.length;
    final convenience = (total * 0.1).round();
    final grand = total + convenience;
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        child: Column(children: [
          ...
          _summaryRow('Format', '2D'),
          const Divider(height: 20),
          _summaryRow('Ticket (${_selectedSeats.length}x)', '₹$total'),
      ...
      Row(children: ['UPI', 'Card', 'Wallet'].map((p) =>
        Container(
          ...
          decoration: BoxDecoration(
            color: p == 'UPI' ? AppColors.primary.withValues(alpha: 0.1) : Colors.white,
            borderRadius: BorderRadius.circular(8),
            border: Border.all(color: p == 'UPI' ? AppColors.primary : AppColors.border)),
          child: Text(p, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700,
              color: p == 'UPI' ? AppColors.primary : AppColors.textSecondary)),
        )
      ).toList()),
        ...
        child: ElevatedButton(
          onPressed: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text('🎬 ${widget.movie.title} booked! Seats: ${_selectedSeats.join(', ')} at $_selectedTime'),
              backgroundColor: AppColors.success,
              duration: const Duration(seconds: 4),
            ));
          },
          style: ElevatedButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
          child: Text('Pay ₹$grand', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
        )),
    ]);
  }
